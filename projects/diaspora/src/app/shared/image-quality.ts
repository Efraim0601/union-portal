...
export interface DocQuality {
  ok: boolean;
  issue: DocIssue | null;
  /** Boîte englobante détectée (fraction 0..1 du cadre), quand des bords ont pu être trouvés —
   *  permet à l'appelant de dessiner un cadrage live qui suit la pièce filmée. */
...
function assessFraming(g: Uint8ClampedArray, w: number, h: number): DocQuality {
  ...
  if (fill < MIN_FILL) return { ok: false, issue: 'too_far', box };
  ...
  if (minX < bandX || maxX > w - 1 - bandX || minY < bandY || maxY > h - 1 - bandY) {
    return { ok: false, issue: 'cut_off', box };
  }
  return { ok: true, issue: null, box };
}
